...
function EditAdmin() {
    ...
    async function submit(e) {
        e.preventDefault()
        try {
            await axios.post("http://localhost:4000/edit", {
            ...
            <div className={styles.container}>
                ...
                <div className={styles.right}>  
                    <div className={styles.form}>
                        ...
                        <div><label>Tên đăng nhập cũ</label></div>
                        ...
                        <div><label>Tên đăng nhập mới</label></div>
                        <div><input type="text" name="newusername" onChange={(e) => {setNewUserName(e.target.value)}}></input></div>
                        <div><label>Mật khẩu mới</label></div>
                        <div><input type="text" name="newpassword" onChange={(e) => {setNewPassword(e.target.value)}}></input></div>
                        <div><input id="submit" type="submit" name="submit" value="Sửa" onClick={submit}></input></div>
                        </form>
                    </div>
                </div>
            </div>
